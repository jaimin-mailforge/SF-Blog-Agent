...
- **No typical AI framings.** `rules/writing.md` section 5a is the single index: the two banned lists, the judgment calls with no checker, and a table of the seven framings other sections already enforce. It is a preventive list, so read it before drafting rather than hunting for a problem in a clean draft.
