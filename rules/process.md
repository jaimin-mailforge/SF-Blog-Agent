...
- **A price behind a billing toggle needs a screenshot of the rendered page as its source.** Fixed on 2026-08-13, after Waalaxy cost two rounds of rework. Extracting the price constants out of the markup is not enough, because a vendor can serve more than one version of the same pricing page. Waalaxy was serving a Monthly, Quarterly -20%, Yearly -50% toggle in the server HTML and a Monthly, Yearly -20% toggle in the browser, which is different cycles at different discounts, and the mismatch is what produced euro figures the page never showed a buyer. So screenshot the toggle in the state you are quoting, note which tab the page opens on, and put both in the brief. Where the versions disagree, the screenshot wins and the article tells the reader to check the toggle rather than printing a discount percentage.
